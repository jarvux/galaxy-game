...
class PlayScene(Scene):
    ...
    def do_update(self, delta_time: float):
        ...
        if self.start and diff >= self.window_cfg["start"]["waiting_time"]:
            ...
            if not self._paused:
                ...
                system_enemy_bullet(self.ecs_world)

                system_screen_bounce(self.ecs_world, self.screen)
                system_screen_player(self.ecs_world, self.screen)
                system_screen_bullet(self.ecs_world, self.screen)

                system_collision_enemy_bullet(self.ecs_world, self.explosion_cfg, self.conttrol_items["score"],
                                              self.conttrol_items["hi-score"])
                system_collision_player_enemy(self.ecs_world, self._player_entity, self.level_01_cfg,
                                              self.player_explosion_cfg,
                                              self.screen, self._num_lives, self)

                system_collision_player_bullet(self.ecs_world, self._player_entity, self.explosion_cfg, self, self.screen)

                system_explosion_kill(self.ecs_world)
                system_player_state(self.ecs_world)
                # TODO: PILAS CREAR DIFERENTES ENEMIGOS
                system_enemy_hunter_random(self.ecs_world, self._player_entity, self.enemies_cfg, self.bullet_cfg)
            ...
            system_enemies_count(self.ecs_world, self.level_01_cfg, self.enemies_cfg, self.screen,
                                 self.conttrol_items["level"])
        else:
    ...
    def do_action(self, action: CInputCommand):
        ...
        if action.name == "PLAYER_FIRE" and self.num_bullets <= self.level_01_cfg["player_spawn"][
            "max_bullets"] and action.phase == CommandPhase.START:
